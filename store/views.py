...
def user_login(request):
    """Customer login allowing 10-digit phone number or username."""
    if request.user.is_authenticated:
        return redirect('home')
    ...
    if request.method == 'POST':
        ...
        if not login_identifier or not password:
            ...
            return render(request, 'registration/login.html', {'next': next_url, 'login_identifier': login_identifier})

        # Check credentials
        user = authenticate(request, username=login_identifier, password=password)

        # If user not found by username, check if phone matches a custom user
        if user is None and hasattr(get_user_model() if 'get_user_model' in globals() else User, 'phone'):
            user_model = get_user_model() if 'get_user_model' in globals() else User
            try:
                user_obj = user_model.objects.get(phone=login_identifier)
